...
bool RH_E32::readParameters(Parameters& params)
{
  ...
  size_t result = _s->readBytes((uint8_t*)&params, sizeof(params)); // default 1 sec timeout
  setOperatingMode(ModeNormal);
...
bool RH_E32::available()
{
  ...
  //Serial.println("RH_E32::available");
    if (!_rxBufValid)
    {
	if (_mode == RHModeTx)
	  return false;
...
bool RH_E32::send(const uint8_t* data, uint8_t len)
{
  ...
  //RHGenericDriver::printBuffer("TX: ", _buf,  len + RH_E32_HEADER_LEN);
