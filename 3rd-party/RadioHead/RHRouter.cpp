...
void RHRouter::addRouteTo(uint8_t dest, uint8_t next_hop, uint8_t state)
{
    uint8_t i;
    /*
    Serial.print("RHRouter::addRouteTo: 0x");
...
RHRouter::RoutingTableEntry* RHRouter::getRouteTo(uint8_t dest)
{
    ...
    //Serial.print("RHRouter::getRouteTo: 0x");
    //Serial.print(dest,HEX);
    ...
    for (i = 0; i < RH_ROUTING_TABLE_SIZE; i++)
		if (_routes[i].dest == dest && _routes[i].state != Invalid){
		    ...
		    Serial.print(", 0x");
    		Serial.println(_routes[i].state,HEX);
    		*/
	    	return &_routes[i];
		}
...
{
    //Serial.println("RHRouter::sendtoWait");
...
{  
    ...
    //Serial.println("RHRouter::recvfromAck");
