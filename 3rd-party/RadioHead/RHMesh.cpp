...
{
    //Serial.println("RHMesh::sendtoWait");
    if (len > RH_MESH_MAX_MESSAGE_LEN){
		//Serial.println("RHMesh::sendtoWait failed1");
    ...
    if (address != RH_BROADCAST_ADDRESS)
    {
		...
		if (!route && !doArp(address)){
			//Serial.println("RHMesh::sendtoWait failed2");
    ...
    int ret = RHRouter::sendtoWait(_tmpMessage, sizeof(RHMesh::MeshMessageHeader) + len, address, flags);
    /*
    if (ret==RH_ROUTER_ERROR_NONE)
    	Serial.println("RHRouter::sendtoWait ok");
    else
		Serial.println("RHRouter::sendtoWait failed3");
		*/
    return ret;
}
...
bool RHMesh::doArp(uint8_t address)
{
    ...
    if (error !=  RH_ROUTER_ERROR_NONE){
		//Serial.println("RHMesh::doArp broadcast failed");
	...
	if (waitAvailableTimeout(timeLeft))
	{
	    ...
	    {
			//Serial.println("RHMesh::doArp recvfromAck success");
			//Serial.println(messageLen, HEX);
			//Serial.println(p->header.msgType, HEX);
			//Serial.println(RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE, HEX);
				   ...
				   && p->header.msgType == RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE)
			{
				...
				//Serial.println("RHMesh::doArp success");
				return true;
			}
	    }
	}
    ...
    //Serial.println("RHMesh::doArp timeout failed");
    return false;
}
...
{     
    ...
    //Serial.println("RHMesh::recvfromAck");
